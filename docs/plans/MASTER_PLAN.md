...
| D10 | **Zero-config decimal determination is IMPOSSIBLE from Vision's text on the target instrument.** The correct behaviour for that class is REFUSAL, not rescue. Determination requires pixel evidence (D4's scanner) behind rectification. See `AUTO_DECIMAL_RESEARCH.md`. | Measured three ways on `Fixtures/ir_gun_display.png` and the corpus: (a) Vision `.accurate` returns `900`/`927` for a display showing 90.0/92.7 with **no dot in any candidate at revisions 1, 2 and 3** — the loss is inside Vision, before DAQPal runs; (b) two 30-frame arms with truths 90.0 and 900 produced **byte-identical** streams — likelihood ratio exactly 1 for all N; (c) on 3.5/4.5-digit panel meters the decimal is set by a **hardware jumper**, so the digit pattern cannot encode it even in principle. | Reopen only if a rectified pixel path (ranks 4–6) demonstrates decimal recovery on a REAL fixture. Note `DAQPal_DEVICE_CONTEXT_RESEARCH.md` §3.5 recommends deleting `SegmentCellScanner`/`DecimalRescue` — **do NOT execute that**; the pixel route is now the only remaining path to genuine determination. |
...
0. **DEVICE TESTING IS BLOCKED — verified 2026-08-03 against the attached iPhone.** The app builds, signs and installs on `Daniphone` (iOS 26.5.2, `00008101-001E44EC1A88001E`) — `** BUILD SUCCEEDED **`. But `xcodebuild test` on device fails for BOTH configurations: `DAQPalTests` and `DAQPalUITests` have no `DEVELOPMENT_TEAM` (only the app target has `56WA4P82Z5`), so neither test bundle can be signed. Release testing is separately blocked by `ENABLE_TESTABILITY = NO` in Release vs 54 `@testable` files. **C5 / Device Day 1 would fail at step 1 as currently written.** Fix: set `DEVELOPMENT_TEAM` on both test targets, and add a third `Benchmark` configuration (Release optimization + testability) rather than enabling testability in `Release` — doing the latter disables cross-module optimization and would make every DD1 number not-the-shipping-config, defeating D5. → WS-C, before DD1
