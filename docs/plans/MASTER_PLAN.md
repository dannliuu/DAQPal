...
| DoD-4 | Standing regression net | Full suite green (**870 baseline — see §5, never a hardcoded number**) + recorded `.baseline` sweeps + Release budget tests, all passing at every integration gate | CI/test runs logged in §5 |
...
| D9 | **PROPOSED — needs Daniel's decision.** `ConfidenceEngine.minimumFusedConfidence` = `lowOCRConfidenceThreshold × TemporalFilter.consistencyThreshold × decimalVetoThreshold × (1 − crossCheckVetoThreshold)` = **0.0375**, written as arithmetic so it tracks its inputs. | It is the *infimum of the gate-permitted region*, so it fires only when a factor bypassed its own gate — an invariant tripwire. **It therefore does NOT close the leak it was written for:** the documented case (ocr 0.35 × decimal 0.50 × crossCheck 0.51 = **0.0892**) still exceeds it and is still accepted. `.lowFusedConfidence` is currently dead in the data. | **Reopen immediately.** Closing the leak needs a POLICY floor ("what confidence is too low to export as trustworthy?"), which is not derivable — same class as the 0.30/0.50 thresholds, whose own comments call them policy. Daniel sets the number. |
...
**Test suite (2026-08-03, after B0, via `xcresulttool`): 870 passed / 1 failed / 2 skipped / 1 expected failure = 874 cases** `[sim iPhone 16e iOS 18.4, Debug, serial]`.
